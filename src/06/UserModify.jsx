...
const User = ({user, onRemove, onToggle, onModify }) => {//props로 객체를 받는 경우 {}로 받음
  ...
  return(
      <>
        <td>{id}</td>
        <td><b style ={{cursor : 'pointer',color : user.selected?'green':'black'}} onClick ={()=>onToggle(id,user)}> {name}</b> </td>
        <td>{email}</td>
        <td>
        <button onClick ={()=>onRemove(id)}>삭제</button> &nbsp;&nbsp;{/* 삭제 버튼 클릭시 id로 remove 함수 호출  */} 
        <button onClick ={()=>onModify(user,id)}>수정</button>{/* 삭제 버튼 클릭시 id로 remove 함수 호출  */} 
        </td>
      </>
    );
}
...
const UserList =({users,onRemove, onToggle, onModify})=>{ //User컴포넌트에 있는 onRemove함수 전달
  return(
  <table>
     <thead>
     <tr>
       <th>연번</th><th>이름</th><th>이메일</th><th></th>
     </tr>
   </thead>
   {
     users.map(
       (user)=>{return (
       <tr><User user = {user} key = {user.id} onRemove={onRemove} onToggle ={onToggle} onModify = {onModify}/></tr>)
      })  //2. User onToggle
...
export default function UserModify(){
   ...
   const onCreate =()=>{ 
    ...
    const onToggle = (id, user)=>{ //2. 1)이름 클릭하면 selected속성 반전 2) input에 현재값 반영하기
     ...
     return (
        <div> {/* 4. input의 name, email로 user객체 요소 렌더링, 컴포넌트에 수정,등록, 삭제 함수 전달 */}
